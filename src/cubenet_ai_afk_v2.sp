...
bool EngIsInBuildMode(int client)
{
    ...
    {
        if (GetEntProp(client, Prop_Send, "m_iCurrentBuild") != 0)
            return true;
    }
...
void Eng_RunBuildSentryFSM(int client, int enemy, int metal)
{
    ...
    if (enemy > 0)
    {
        ...
        if (dist < 500.0)
            ...
            g_EngDangerUntil[client] = now + 2.5;
    }
    ...
    // Not enough metal → go to dispenser
    if (metal < 130)
    {
    ...
    if (g_BuildState[client] == 0)
    {
        ...
        g_NextBuildCheck[client] = now + 1.8;   // timeout for entering build mode
        g_EngStageUntil[client]  = now + 6.0;   // overall placement timeout
        return;
    }

    //--------------------------------------------------
    // State 1: Inside build mode → place
    //--------------------------------------------------
    if (g_BuildState[client] == 1)
    {
        // Never entered build mode
        if (now >= g_NextBuildCheck[client] && !EngIsInBuildMode(client))
        {
            FakeClientCommand(client, "build 0");
            g_IsInBuildMode[client] = false;
            g_BuildState[client]    = 0;
            g_NextBuildTry[client]  = now + 2.5;
            return;
        }

        // Detect entry into build mode
        if (!g_IsInBuildMode[client] && EngIsInBuildMode(client))
        {
            g_IsInBuildMode[client] = true;
            g_EngStageUntil[client] = now + 4.5;
        }

        if (!g_IsInBuildMode[client])
        {
            // Still waiting – look down, stay still
            float ang[3];
            GetClientEyeAngles(client, ang);
            ang[0] = 25.0;
            g_LookAt[client][0] = ang[0];
        ...
        // ---- In build mode: stop completely and place ----
        float ang[3];
        GetClientEyeAngles(client, ang);
        ang[0] = 28.0;
        g_LookAt[client][0] = ang[0];
        ...
        if (FindFriendlySentry(client) > 0)
        {
            g_BuildState[client]     = 0;
            g_IsInBuildMode[client]  = false;
            g_NextBuildTry[client]   = now + 4.0;
            g_EngDeployGuard[client] = now + 3.0;
            return;
        }

        // Timeout → cancel + small nudge
        if (now > g_EngStageUntil[client])
        {
            ...
            float pos[3];
            GetClientAbsOrigin(client, pos);
            pos[0] += GetRandomFloat(-45.0, 45.0);
            pos[1] += GetRandomFloat(-45.0, 45.0);
            TeleportEntity(client, pos, NULL_VECTOR, NULL_VECTOR);
        }
        return;
    }

    // Legacy state 2 (safety)
    if (g_BuildState[client] == 2)
    {
        ...
        {
            g_BuildState[client] = 0;
            g_NextBuildTry[client] = now + (FindFriendlySentry(client) > 0 ? 8.0 : 3.0);
        }
    }
}
...
void AI_EngineerThink(int client, int enemy)
{
    ...
    // Hard stop movement while actively placing
    ...
    switch (g_EngStage[client])
    {
        ...
        case ENG_STAGE_DEPLOY:
        {
            ...
            // Force into build mode if needed
